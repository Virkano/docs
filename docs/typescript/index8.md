...
```bash
pnpm add @types/jquery --save-dev
```

可以在这个页面搜索你需要的声明文件。

<CustomLink title="搜索你需要的声明文件" href="https://www.typescriptlang.org/dt/search?search=" />
...
与普通的 npm 模块不同，```@types``` 是统一由<CustomLink title="DefinitelyTyped" href="https://github.com/DefinitelyTyped/DefinitelyTyped/" /> 管理的。要将声明文件发布到 ```@types``` 下，就需要给 ```DefinitelyTyped``` 创建一个 pull-request，其中包含了类型声明文件，测试代码，以及 ```tsconfig.json``` 等。
