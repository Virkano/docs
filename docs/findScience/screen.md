...
// * 屏幕缩放适配（两边留白）
export function usePreviewFitScale(width, height, scaleDom, callback) {
  // * 画布尺寸（px）
  ...
  const calcRate = () => {
    ...
    if (scaleDom) {
      if (currentRate > baseProportion) {
        ...
        // 表示更高
        scale.height = Number.parseFloat(
...
// *  X轴撑满，Y轴滚动条
export function usePreviewScrollYScale(width, height, scaleDom, callback) {
  // * 画布尺寸（px）
  const baseWidth = width
  const baseHeight = height
...
// *  Y轴撑满，X轴滚动条
export function usePreviewScrollXScale(width, height, scaleDom, callback) {
  // * 画布尺寸（px）
  const baseWidth = width
  const baseHeight = height
...
// * 变形内容，宽高铺满
export function usePreviewFullScale(width, height, scaleDom, callback) {
  // * 默认缩放值
  const scale = {
    width: 1,
    height: 1,
